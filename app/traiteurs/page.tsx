...
export default function TraiteursPage() {
  ...
  useEffect(() => {
    async function fetchEstablishments() {
      ...
      const { data, error } = await supabase
        .from("establishments")
        .select("*")
        .eq("type", "traiteur"); // filtre par type "hotel"
  ...
  if (loading) return <p>Chargement des traiteurs...</p>;
  if (error) return <p className="text-red-600">Erreur : {error}</p>;
  if (establishments.length === 0) return <p>Aucun traiteur trouvé.</p>;

  return (
    <div className="max-w-4xl mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6">Traiteurs</h1>
